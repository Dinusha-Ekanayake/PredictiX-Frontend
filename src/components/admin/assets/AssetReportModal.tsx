...
export default function AssetReportModal({ isOpen, onClose, assetId, assetName }: Props) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        // Parallel fetch: asset + fleet summary + predictions + maintenance + tickets + sensor
        ...
        // Sequential: predictions, maintenance, tickets, sensor
        const [predRes, maintRes, ticketRes, sensorRes, warehouseRes] = await Promise.all([
          fetch(`${API_URL}/predictions/failure/${assetId}`).catch(() => null),
          fetch(`${API_URL}/maintenance/?asset_id=${assetId}`).catch(() => null),
        ...
        const maintList  = maintRes?.ok     ? await maintRes.json()     : [];
        const ticketList = ticketRes?.ok    ? await ticketRes.json()    : [];
        const sensorList = sensorRes?.ok    ? await sensorRes.json()    : [];
        const warehouse  = warehouseRes?.ok ? await warehouseRes.json() : null;

        const sensor = Array.isArray(sensorList) ? sensorList[0] : null;
        const latestPred = Array.isArray(pred) ? pred[0] : pred;
        ...
        const topRisk   = (fleet.topRiskAssets || []).slice(0, 5);

        // Status + vehicle distribution from asset list (approximated from fleet)
        const statusDist: Array<{ name: string; count: number }> = [];
        const vehicleDist: Array<{ name: string; count: number }> = [];

        // Build report data
        const health_score     = latestPred?.health_score ?? null;
        const failure_prob     = latestPred?.failure_probability != null
          ? Math.round(latestPred.failure_probability * 100 * 10) / 10
          : null;
        const risk_level       = latestPred?.risk_level ?? null;
        ...
        const data: AssetReportData = {
          ...
          warehouseName: warehouse?.name ?? asset.warehouse_id?.slice(0, 8) ?? "—",
          reportDate: new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" }),
          asset,
          health_score:               health_score != null ? Math.round(health_score * 10) / 10 : undefined,
          failure_probability:        failure_prob ?? undefined,
          risk_level:                 risk_level ?? undefined,
          days_until_maintenance:     days_until_maint,
          predicted_maintenance_date: pred_maint_date,
          estimated_cost:             latestPred?.estimated_cost ?? undefined,
          currency:                   "LKR",
          top_explanations:           latestPred?.top_explanations ?? undefined,
          ...
          maintenanceMetrics: {
            ...
            total_cost:         mArr.reduce((s: number, m: any) => s + (parseFloat(m.cost_amount) || 0), 0),
            total_downtime_hours: Math.round(mArr.reduce((s: number, m: any) => s + (parseFloat(m.downtime_hours) || 0), 0) * 10) / 10,
          },
          tickets: tArr,
          ticketMetrics: {
          ...
          insights: {
            executive_summary: latestPred
              ? `${asset.asset_name} has a health score of ${Math.round(health_score ?? 0)}% with a failure probability of ${failure_prob ?? 0}%. Risk level is classified as ${risk_level ?? "unknown"}.`
              : "No prediction data available for this asset.",
            recommendations: {
              critical: health_score != null && health_score < 60
                ? ["Schedule immediate maintenance inspection", "Review critical sensor readings"]
                : [],
              high: health_score != null && health_score < 75
                ? ["Plan preventive maintenance within 14 days"]
                : [],
              medium: ["Monitor sensor readings regularly", "Ensure service schedule is up to date"],
            },
            conclusion: latestPred
              ? `Based on current predictions, ${asset.asset_name} ${(health_score ?? 100) < 60 ? "requires urgent attention" : "is within acceptable operating parameters"}. ${days_until_maint != null ? `Maintenance is predicted in ${days_until_maint} days.` : ""}`
              : "Run AI prediction to get actionable insights for this asset.",
          },
        };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      toast.success("Report opened for printing — choose 'Save as PDF'");
    } catch (e: any) {
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <KpiCard icon={AlertTriangle} label="Failure Probability"
                  value={fp != null ? `${fp}%` : "—"} accent="bg-orange-500/15" />
                <KpiCard icon={Shield} label="Risk Level"
                  value={rl ?? "—"} accent="bg-violet-500/15" />
              ...
              <Section title="1. Asset Overview" icon={BarChart3}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                  ...
                  <div>
                    <Field label="Criticality Score"  value={String(asset?.criticality_score ?? "—")} />
                    ...
                    <Field label="Predicted Maint."   value={reportData.predicted_maintenance_date?.slice(0,10) ?? "—"} />
              ...
              <Section title="4. AI Insights" icon={Bot}>
                <div className="flex items-start gap-3 rounded-xl border border-teal-500/20 bg-teal-500/5 p-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-teal-300">Full AI Analysis Included in PDF</p>
