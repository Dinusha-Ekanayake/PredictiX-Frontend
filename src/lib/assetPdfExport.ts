...
export interface AssetReportData {
  ...
  asset: {
    asset_name?: string; asset_code?: string; registration_number?: string; vin?: string;
    warehouse_id?: string;
...
function buildPage(content: string, warehouse: string, section: string, pageNum: number, total: number): string {
  return `<div class="page">
    <div style="border:2px solid ${C.teal};border-radius:4px;display:flex;flex-direction:column;overflow:hidden">
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .page{page-break-after:always;background:white}
...
function generateAssetReportHTML(data: AssetReportData): string {
  ...
  const p3=buildPage(`
    ...
    ${assetKpis}
    ${healthGauge ? `<div style="margin-bottom:13px">${chartBox("ASSET HEALTH & PREDICTION",`
    ...
    `,"Figure 2.1 — Health score and prediction data")}</div>` : ""}
  ...
  const p5=buildPage(`
    ...
    ${sRows.length
      ? infoTable(sRows)
      : highlightBox("SENSOR STATUS","No sensor readings available for this asset. Connect to the PredictiX monitoring system to start receiving data.",C.slate)}

    ${sensorHealthFields.length
      ? `${subH("4.2 Component Health")}${chartBox("COMPONENT HEALTH LEVELS",sensorHealthBars(sensorHealthFields,440),"Figure 4.1 — Component health as percentage")}`
      : ""}
    ...
    ${maintenance.length
      ? subH("4.4 Recent Maintenance Events")+dataTable(["Date","Type","Description","Cost ("+cur+")","Downtime"],maintRows,"Figure 4.3 — Most recent maintenance events")
    ...
    ${ticketDonut ? chartBox("TICKET STATUS DISTRIBUTION",ticketDonut,"Figure 5.1 — Ticket distribution by status") : ""}

    ${tickets.length
      ? subH("5.2 Recent Tickets")+dataTable(["Ticket ID","Title","Priority","Status","Created"],ticketRows,"Figure 5.2 — Most recent support tickets")
    ...
    ${!recs.critical.length&&!recs.high.length&&!recs.medium.length
      ? highlightBox("RECOMMENDATIONS","No specific recommendations. Run AI prediction to get asset-specific guidance.",C.slate)
      : ""}

    ${insights.conclusion
      ? subH("5.4 Conclusion")+highlightBox("EXECUTIVE CONCLUSION",insights.conclusion,C.teal)
      : ""}
